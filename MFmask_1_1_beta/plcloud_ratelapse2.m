...
%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
function [zen,azi,cptm,Temp,t_templ,t_temph,WT,Snow,Cloud,Shadow,...
    dem,dem_b,dim,mask,ul,resolu,zc]=plcloud_ratelapse2(fullfile_path,image,demtype,cldprob,num_Lst)
    % resolution of Fmask results
    fprintf('Loading input data.\n');
    [dem,slope,aspect,Temp,data,dim,ul,zen,azi,zc,satu_B1,satu_B2,satu_B3,resolu]=nd2toarbt_xml(fullfile_path,image,demtype);
...
%         return;
%     end
    WT=WT&(slope<10); % DEM is always available when arriving at here.
    WT(mask==0)=255;
    ...
    cptm=100*sum(idclr(:))/sum(mask(:));% percent of clear pixel
    ...
    if cptm <= 0.1 % no thermal test => meanless for snow detection (0~1)
        ...
        if isequal(dem,0)  % when no DEMs, back to Fmask rountine.
            dem_b=0;
            Tempd=double(Temp);
        else % BT normalization
    ...
    %    wTemp_prob(wTemp_prob > 1) = 1;
       ...
       %% temperature test (over land)
        ...
        % test trigger.
%         outputdir='E:\Data\Cloud Cover Assessment Validation Data Mountains\lapse_rate_ans\ans_cloud_laspe_rate.txt'; %for test
%         output=[num2str(rate_lapse),' ',num2str(t_templ),' ',num2str(t_temph),' ',...
%             num2str(t_templ_ori),' ',num2str(t_temph_ori)]; %for test
%         dlmwrite(outputdir,output,'-append',  'delimiter', ' ','newline', 'pc'); %for test
