function [dem_out,slope_out,aspect_out,im_th,TOAref,ijdim_ref,ul,zen,azi,zc,B1Satu,B2Satu,B3Satu,resolu]=nd2toarbt_xml(fullfile_path,filename,demtype)

% convert DNs to TOA ref and BT
% Revisions:
% Interpolate missing values for DEMs (Shi 09/12/2017)
% Automately download DEM data derived from TOPOTOOLBOX package(Schwanghart, and Scherler,2014) (Shi 09/07/2017)
...
%
% [dem,im_th,TOAref,ijdim_ref,ul,zen,azi,zc,B1Satu,B2Satu,B3Satu,resolu]=nd2toarbt(filename)
% Where:
% Inputs:
% filename='L*MTL.txt';
% Outputs:
% read dem modified by QS 2016/2/22
...
% fprintf('Reading in header information & TIF images\n');
...
if Lnum >= 4 && Lnum <= 7
    ...
    % Read in all bands
    % Band1
    n_B1=dir([fullfile_path,'*B1*.TIF']);
    im_B1=single(imread([fullfile_path,n_B1(1).name]));
    % Band2
    n_B2=dir([fullfile_path,'*B2*.TIF']);
    im_B2=single(imread([fullfile_path,n_B2(1).name]));
    % Band3
    n_B3=dir([fullfile_path,'*B3*.TIF']);
    im_B3=single(imread([fullfile_path,n_B3(1).name]));
    % Band4
    n_B4=dir([fullfile_path,'*B4*.TIF']);
    im_B4=single(imread([fullfile_path,n_B4(1).name]));
    % Band5
    n_B5=dir([fullfile_path,'*B5*.TIF']);
    im_B5=single(imread([fullfile_path,n_B5(1).name]));
    % Band6
    if Lnum==7
        n_B6=dir([fullfile_path,'*B6*1*.TIF']);
    else
        n_B6=dir([fullfile_path,'*B6*.TIF']);
    end
    im_th=single(imread([fullfile_path,n_B6(1).name]));
    ...
    % Band7
    n_B7=dir([fullfile_path,'*B7*.TIF']);
    im_B7=single(imread([fullfile_path,n_B7(1).name]));
    ...
    fprintf('Calculating TOA ref & BT from DNs.\n');
...
elseif Lnum == 8
    ...
    % Band2
    n_B2=dir([fullfile_path,'*B2*.TIF']);
    im_B2=single(imread([fullfile_path,n_B2.name]));
    % Band3
    n_B3=dir([fullfile_path,'*B3*.TIF']);
    im_B3=single(imread([fullfile_path,n_B3.name]));
    % Band4
    n_B4=dir([fullfile_path,'*B4*.TIF']);
    im_B4=single(imread([fullfile_path,n_B4.name]));
    % Band5
    n_B5=dir([fullfile_path,'*B5*.TIF']);
    im_B5=single(imread([fullfile_path,n_B5.name]));
    % Band6
    n_B6=dir([fullfile_path,'*B6*.TIF']);
    im_B6=single(imread([fullfile_path,n_B6.name]));
    % Band7
    n_B7=dir([fullfile_path,'*B7*.TIF']);
    im_B7=single(imread([fullfile_path,n_B7.name]));
    % Band9
    n_B9=dir([fullfile_path,'*B9*.TIF']);
    im_B9=single(imread([fullfile_path,n_B9.name]));
    % Band10
    n_B10=dir([fullfile_path,'*B10*.TIF']);
    im_B10=single(imread([fullfile_path,n_B10.name]));
...
% automately process DEM data. If dem is unavaible, this code automately download the
% corrponding DEM data from SRTM.

% 1th step: DEM is avalaible or not.
% if DEM is available, directly load this DEM, and make sure the DEM is
% right for the current Landsat.
n_dem=dir([fullfile_path,'*_dem.TIF']);
if size(n_dem,1)==0 % NO DEM
    try
        % automately download DEM.
        tic
        if isequal(demtype,'SRTMGL1')
            fprintf('30m SRTM  will be automatedly downloaded.\n');
        end
        if isequal(demtype,'SRTMGL3')
            fprintf('90m SRTM will be automatedly downloaded.\n');
        end
        fprintf('This will take some time according to your network speed.\n');
        tempfile_path=[tempname '.tif'];
        ...
        time=toc;
        time=time/60;
        fprintf('%.2f minutes for downloading DEMs.\n',time);
        fprintf('The DEM data was locally saved. You can find it at the current folder, named with the end of "_dem.TIF".\n');
    catch % maybe bad network.
    % if no network and DEMs, directly conduct the Fmask, excluding of
    % uzility of cloud shadow detection with aid of neighboring clouds.
        warning('readopentopo:nd2toarbt_xml',['Fail to download DEMs.\n'...
            'Pleas make sure your network always available and \n'...
            'Matlab version higher than R2014b if you haven''t local DEMs.\n' ...
            'Or the corresponding DEM data needs to be manually downloaded,\n'...
            'mosaicked, projected, and resampled to Landsat''s resolution\n'...
            'and extent, and resaved as TIFF file named with the end of "_dem.TIF".\n']);
        aspect_out=0;
        slope_out=0;
        dem_out=0;
        return;
    ...
    dem_temp = GRIDobj([fullfile_path,n_dem.name]);
end

% 2th step: reproject, and resample to the corresponding Landsat extent.

target_landsat = GRIDobj([fullfile_path,n_B2(1).name]);
if ~isequal(dem_temp.size,target_landsat.size) % when DEM is NOT same as Landsat, CLIP it.
    dem = reproject2utm(dem_temp,target_landsat,'method','nearest');
    GRIDobj2geotiff(dem,[fullfile_path,filename(1:end-8),'_dem.TIF'])
else
    dem = dem_temp;
end

% dem=inpaintnans(dem,'nearest'); fail to work well.
clear dem_temp;

% 3th step: calcualte aspect and slope.
aspect_dem=aspect(dem);
aspect_out=single(aspect_dem.Z);
slope_dem=arcslope(dem,'deg');
slope_out=single(slope_dem.Z);
dem_out=dem.Z;
% interpolate missing values using 0.
dem_out(isnan(dem_out(:)))=0;

% n_dem=dir([fullfile_path,'*_dem.tif']);
% dem=single(imread([fullfile_path,n_dem.name]));
% 
% n_slope=dir([fullfile_path,'*_slope.tif']);
% slope=single(imread([fullfile_path,n_slope.name]));
% 
% n_aspect=dir([fullfile_path,'*_aspect.tif']);
% aspect=single(imread([fullfile_path,n_aspect.name]));
